...
bpy.ops.wm.open_mainfile(filepath=str(E/'alec-cube-v4-1.blend'));S=bpy.context.scene
...
S['source_pcb_sha256']=hashlib.sha256((REPO/'boards/alec-main/alec-main.kicad_pcb').read_bytes()).hexdigest()
source={k:{'pcb':'boards/alec-'+k+'/alec-'+k+'.kicad_pcb','pcb_sha256':hashlib.sha256((REPO/'boards'/('alec-'+k)/('alec-'+k+'.kicad_pcb')).read_bytes()).hexdigest(),'glb_sha256':hashlib.sha256((R/'sources'/f'{k}.glb').read_bytes()).hexdigest(),'bounds_mm':bounds(meshes[k])} for k in meshes}
...
S.camera=bpy.data.objects['CAM Exterior'];bpy.ops.wm.save_as_mainfile(filepath=str(R/'alec-cube-v4-2.blend'))
...
S.render.filepath=str(R/'internals.png');bpy.ops.wm.save_as_mainfile(filepath=str(R/'alec-cube-v4-2-internals.blend'))
