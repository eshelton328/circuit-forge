...
bpy.ops.wm.open_mainfile(filepath=str(R/'bedroom-cube-v4-2.blend'));S=bpy.context.scene;deps=bpy.context.evaluated_depsgraph_get()
checks=[]
...
def snapshot():
 ...
 return result
updated=snapshot();bpy.ops.wm.open_mainfile(filepath=str(E/'bedroom-cube-v4-1.blend'));baseline=snapshot();changes=[n for n in set(updated)|set(baseline) if updated.get(n)!=baseline.get(n)]
...
report=dict(status='NOMINAL_GEOMETRY_PASS' if all(c['passed'] for c in checks) else 'GEOMETRY_FAILURE',checks=checks,failed_checks=[c['name'] for c in checks if not c['passed']],tested_blend_sha256=hashlib.sha256((R/'bedroom-cube-v4-2.blend').read_bytes()).hexdigest(),manufacturing_release=False,physical_qualification_performed=False)
