...
**Validation follow-up (6 September):** the user has specified three AA cells, alkaline or lithium; the precise lithium type is awaiting clarification. See the [operating-envelope update](review/physical-validation/operating-envelope-update.md), [recorded requirements](analysis/operating-requirements.json), and [bench plan with test-pad mapping](review/physical-validation/bench-plan.md). The follow-up adds Q1 heat, 30 three-AA source scenarios and finer C3 extraction. The earlier thermal report omits Q1 loss and is not a complete system heat budget. Current Python verification is 92 passed, one integration skip.
