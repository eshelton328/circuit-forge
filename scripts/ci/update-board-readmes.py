...
Handles fenced regions (delimited by HTML comment markers):
  ...
  <!-- spice-regression-start --> … <!-- spice-regression-end -->
    Boards with sim.yml: summary table from docs/spice-report.metrics.json when present
    (generated by run_sim / main-branch workflow). Otherwise a short placeholder.
...
def _format_measure_bounds_cell(item: dict[str, object]) -> str:
    lo = item.get("bounds_min")
    hi = item.get("bounds_max")
    parts: list[str] = []
    if lo is not None:
...
def _build_spice_section(board_name: str, docs_dir: Path) -> str:
    """Summarize last ngspice regression from metrics sidecar (optional on-disk)."""
    lines: list[str] = [
        SPICE_START,
        "## SPICE regression (ngspice)",
        "",
        "_Auto-generated when `docs/spice-report.metrics.json` is present (see `sim.yml` and "
        "[`sim/README.md`](../../sim/README.md))._",
        "",
    ]

    metrics_path = docs_dir / "spice-report.metrics.json"
    if not metrics_path.is_file():
        lines += [
            "_No committed metrics file yet._ After the next successful **`update-readmes`** run on "
            "`main` that includes the SPICE job, this section will list scenario results. "
            "Full reports also appear as CI artifacts under **`spice-boards`** on pull requests.",
            "",
            SPICE_END,
        ]
        return "\n".join(lines)
    ...
    try:
        ...
        lines += ["_Could not read `docs/spice-report.metrics.json`._", "", SPICE_END]
        return "\n".join(lines)

    schema_v = doc.get("metrics_schema_version", 0)
    overall = doc.get("pass")
    gen_at = doc.get("generated_at")
    measures_raw = doc.get("measures")
    if not isinstance(measures_raw, list):
        measures_raw = []

    pass_str = "PASS" if overall is True else "FAIL" if overall is False else "unknown"
    refresh = str(gen_at) if gen_at else "(timestamp not in file)"
    lines += [
        f"**Last metrics refresh:** {refresh} · **Overall:** {pass_str} "
        f"(schema v{schema_v})",
        "",
        f"[Full report (`docs/spice-report.md`)](docs/spice-report.md)",
        "",
    ...
    for item in measures_raw:
        ...
        mid = str(item.get("measure_id", ""))
        label = item.get("display_title")
        measure_col = str(label).strip() if isinstance(label, str) and label.strip() else mid
        val = str(item.get("value_str", "—"))
...
def update_readme(board_dir: Path) -> bool:
    ...
    if (board_dir / "sim.yml").is_file() and docs_dir.is_dir() and text.find(SPICE_START) != -1:
        section = _build_spice_section(board_dir.name, docs_dir)
        text = _replace_section(text, SPICE_START, SPICE_END, section)
