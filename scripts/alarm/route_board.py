...
import sys
D=Path(sys.argv[1]).resolve();pcb=D/(D.name+'.kicad_pcb');b=p.LoadBoard(str(pcb))
...
for pd in pads:
 ...
 if b.GetCopperLayerCount()==4 and ref not in ['J5','J6','J7']:continue
...
def order(n):
 ...
 return (0 if name=='/SW_ON' else 1 if name.startswith('/LED_') else 2 if name.startswith('/BTN_') else 3 if name in power else 4,-sum(len(g) for g in ng[n]))
failed=[];done=0
for net in sorted(ng,key=order):
 groups=ng[net];name=b.FindNet(net).GetNetname()
 if len(groups)<2:continue
 width=.15 if NL==3 else .2;vd=.55 if NL==3 else .6;drill=.25 if NL==3 else .3
  ...
  for gi,group in enumerate(groups):
   for aa in tree:
    ...
    for cc in group:
     ...
     choices.append((dist,gi,a,c,0 if aa.IsOnLayer(p.F_Cu) else NL-1,0 if cc.IsOnLayer(p.F_Cu) else NL-1))
  found=False
