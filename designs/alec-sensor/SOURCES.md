...
Manufacturer data and upstream documentation inform S0. Simulation and future test requirements are engineering proposals, not vendor qualification results.
...
Third-party PDFs and the new radar STEP were inspected locally but are not redistributed here. The CAD contains the exact C header envelope; it is not a complete mounted sensor assembly. Retrieval hashes are in [source-hashes.json](review/source-hashes.json). Only source links and extracted dimensional findings are committed.
