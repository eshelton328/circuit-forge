#!/usr/bin/env python3
"""Create the schematic-stage sensor from the reviewed ALEC circuit blocks.
No PCB, fabrication package or inherited qualification reports are generated.
"""
...
r.append(node(f'(title_block (title "ALEC Sensor — schematic study") (date "2026-09-07") (rev "S0 STUDY"))'))
...
add_symbol(r,symbol_def('Switch','SW_SPST'),'SW1','POWER — SELECT DC RATING','',121.92,40.64,{'1':'VBAT','2':'VBAT_SW'},NAME,mpn='TBD_MECHANICAL_DC_SWITCH')
...
note(r,'Schematic study only. Switch, protection, connector retention,\nradome and wet-room behavior must be resolved before PCB release.',270,248,1.27)
...
monitor.append(node('(title_block (title "ALEC Sensor — power monitoring") (date "2026-09-07") (rev "S0 STUDY"))'))
...
r.append(node('(title_block (title "ALEC Sensor — switched radar interface") (date "2026-09-07") (rev "S0 STUDY"))'))
...
add_symbol(r,symbol_def('Connector_Generic','Conn_01x05'),'J3','LD2410C — INTERNAL','Connector_PinSocket_2.54mm:PinSocket_1x05_P2.54mm_Vertical',215.9,76.2,{'1':'LD_TX','2':'LD_RX','3':'LD_OUT','4':'GND','5':'5v'},NAME,'https://www.hlktech.com/en/Goods-239.html','TBD_RETAINED_RADAR_SOCKET')
...
for ref,value,x,y,rail in [('C35','10uF',215.9,114.3,'5v'),('C36','0.1uF',248.92,114.3,'5v'),('C37','0.1uF',149.86,139.7,'3v3')]:
 add_symbol(r,symbol_def('Device','C'),ref,value,'Capacitor_SMD:C_0603_1608Metric',x,y,{'1':rail,'2':'GND'},NAME,mpn='TBD_DC_BIAS_CAPACITOR')
...
(DEST/'fp-lib-table').write_text((BASE/'fp-lib-table').read_text().replace('${KIPRJMOD}/footprints','${KIPRJMOD}/../../boards/esp32s3-devkit-5v/footprints'))
...
print('Generated schematic study:',DEST)
